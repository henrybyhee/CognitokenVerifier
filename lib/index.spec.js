...
describe("Test Step 1: Ensure JWT Structural Integrity", () => {
    it("Should return True if string has 3 parts", () => {
        ...
        chai_1.expect(utils_1.hasThreeSections(token)).equal(true);
    });
    it("Should return False if string has less than 3 parts", () => {
        const token = "AAA.BBB";
        chai_1.expect(utils_1.hasThreeSections(token)).equal(false);
    });
});
describe("Test Step 3: Verify Claim", () => {
    it("Should verify Access token payload.", () => {
        ...
        chai_1.expect(utils_1.verifyClaims("", "Cognito", payload)).equal(true);
    });
    it("Should verify Id Token payload", () => {
        ...
        chai_1.expect(utils_1.verifyClaims("RECIPIENT", "Cognito", payload)).equal(true);
    });
    it("Should not verify if token is expired.", () => {
        ...
        chai_1.expect(utils_1.verifyClaims("", "Cognito", payload)).equal(false);
    });
    it("Should throw error if token use is neither 'access' nor 'id'", () => {
        ...
        chai_1.expect(() => utils_1.verifyClaims("", "Cognito", payload)).to.throw(error_1.CognitokenError);
    });
});
