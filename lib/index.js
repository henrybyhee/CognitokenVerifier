...
var composePromises = function (initialValue) {
    ...
    return functions.reduce(function (accum, func) { return Promise.resolve(accum).then(func); }, initialValue);
};
...
var retrieveBufferFromMap = function (pemMap) { return function (decoded) {
    ...
    var filteredKey = pemMap.filter(function (map) { return map.key === kid; });
 ...
 */
var JWTClaimVerify = function (clientIds, issuer, timeNow, accessType) { return function (payload) {
    var condition = clientIds.reduce(function (prev, curr) { return prev || (payload.aud === curr); }, false) &&
        payload.iss === issuer &&
        payload.exp > timeNow &&
