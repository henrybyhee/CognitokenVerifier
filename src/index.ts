...
const composePromises = (initialValue: string, ...functions) => functions.reduce((accum, func) => Promise.resolve(accum).then(func), initialValue)
...
const retrieveBufferFromMap = (pemMap: IRSAMap[]) => (decoded: IRSAToken) => {
...
const JWTSignatureValidate = (idToken: string) => (pem: Buffer) => {
    return jwtVerifyPromise(idToken, pem, { algorithms: ['RS256'] })
        .catch((err: Error) => { throw new CognitokenError(err.name, err.message) });
}

const JWTClaimVerify = (clientId: string, issuer: string, timeNow: Number, accessType: string) => (payload: IPayload) => {
    const condition = payload.aud === clientId && payload.iss === issuer && payload.exp > timeNow && payload.token_use === accessType;
    if (!condition) {
        throw new CognitokenError('ClaimVerifyError', 'Verification failed: Wrong aud, iss or exp');
    }
    return payload;
}
...
export class CognitokenVerifier {
     ...
     */
    verify(token: string, use: string) {
        const timeNow = new Date().getTime() / 1000;
        return composePromises(
            token,
            JWTStructuralCheck,
            decodeJWT, hasHeaderAndKID, retrieveBufferFromMap(this.keyBufferMap), JWTSignatureValidate(token),
